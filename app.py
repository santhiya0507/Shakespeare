...
# Resolve absolute path for the SQLite DB next to this file to avoid CWD issues
...
def init_db():
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS biographies (
        id {pk_type},
        title TEXT NOT NULL,
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS daily_quotes (
        id {pk_type},
        quote TEXT NOT NULL,
        author TEXT,
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS listening_content (
        id {pk_type},
        title TEXT NOT NULL,
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS observation_content (
        id {pk_type},
        title TEXT NOT NULL,
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS writing_topics (
        id {pk_type},
        topic TEXT NOT NULL,
        description TEXT,
        created_by INTEGER,
    ...
    # Ensure new columns exist on older databases (SQLite only)
    if not USE_POSTGRES:
        existing_cols = [r[1] for r in c.execute('PRAGMA table_info(tasks)').fetchall()]
    else:
        # For PostgreSQL, check columns differently
        c.execute("SELECT column_name FROM information_schema.columns WHERE table_name='tasks'")
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS user_completions (
        id {pk_type},
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS user_streaks (
        id {pk_type},
        user_id INTEGER NOT NULL,
    ...
    c.execute(f'''CREATE TABLE IF NOT EXISTS speaking_attempts (
        id {pk_type},
        user_id INTEGER NOT NULL,
    ...
    if not c.fetchone():
        ...
        c.execute(f"INSERT INTO admins (username, password_hash) VALUES ({placeholder}, {placeholder})", 
                 ('admin', admin_password_hash))
        if USE_POSTGRES:
            c.execute("SELECT currval('admins_id_seq')")
        ...
        for title, name, content, profession, created_by in sample_biographies:
            c.execute(f"INSERT INTO biographies (title, person_name, content, profession, created_by) VALUES ({placeholder}, {placeholder}, {placeholder}, {placeholder}, {placeholder})",
        ...
        for title, audio_file, transcript, robot_character, created_by in sample_listening:
            c.execute(f"INSERT INTO listening_content (title, audio_file, transcript, robot_character, created_by) VALUES ({placeholder}, {placeholder}, {placeholder}, {placeholder}, {placeholder})",
        ...
        for title, video_url, questions, answers, created_by in sample_observation:
            c.execute(f"INSERT INTO observation_content (title, video_url, questions, correct_answers, created_by) VALUES ({placeholder}, {placeholder}, {placeholder}, {placeholder}, {placeholder})",
        ...
        for topic, description, created_by in sample_topics:
            c.execute(f"INSERT INTO writing_topics (topic, description, created_by) VALUES ({placeholder}, {placeholder}, {placeholder})",
